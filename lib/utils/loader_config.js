...
LoaderConfig.parseConfig = function(configFile, charset) {
    ...
    Ast.walk(ast, 'stat', function(stat) {
        ...
        aliasAst && aliasAst.forEach(function(item) {
            alias[item[0]] = item[1][1];
        });

        pathsAst && pathsAst.forEach(function(item) {
            paths[item[0]] = item[1][1];
        });
